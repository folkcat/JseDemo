...
public class UDPClientB {
	...
	public static void main(String[] args) {
		try {
			...
			DatagramPacket recpack = new DatagramPacket(buf,
					buf.length);
	...
	private static void sendMessage(String host, String port,
		...
		try {
			...
			for (;;) {
				String message = "I am master clientB i'm trying hole punch";
				byte[] sendbuf = message.getBytes();
	...
	private static void receive(DatagramSocket client) {
		try {
			for (;;) {
				...
				String reportMessage = "I am client B punch success!";

				// 发送消息
				sendMessage(reportMessage, port, address,
						client);
			}
		} catch (Exception e) {
			e.printStackTrace();
